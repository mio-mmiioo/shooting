...
Camera::Camera()
{
	...
	lookPosition_ = VECTOR3(0, 1.5, 0);
	freeDistance_ = 1.0f;

	cameraPosition_ = VECTOR3(0.5, 1, -3);
...
{
	freeTransform_.position_ = VECTOR3(0, 0, -3);
}
...
void Camera::ThirdCamera()
{
	...
	//VECTOR3 playerHeadPos = VECTOR3(0, 1.5f, 0);
	//VECTOR3 camPos = VECTOR3(0, 0, -0.5f) * MGetRotX(rot.x) * MGetRotY(rot.y);

	//cameraPosition_ = lookPosition_ + camPos + playerHeadPos;
	//targetPosition_ = lookPosition_ + VECTOR3(0, 2.0f, 0); // プレイヤーの頭より少し上を見るように
...
{
	cameraPosition_ = VECTOR3(0, 0, -20);
	targetPosition_ = VECTOR3(0, 0, 0);
}
